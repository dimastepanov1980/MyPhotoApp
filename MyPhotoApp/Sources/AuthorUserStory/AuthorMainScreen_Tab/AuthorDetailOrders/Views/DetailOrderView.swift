    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                if let instagramLink = viewModel.order.customerContactInfo.instagramLink, !instagramLink.isEmpty {
                    Button(action: {
        ...
        .confirmationDialog("Change Status", isPresented: $showActionSheet) {
            ...
            let userOrders = DbOrderModel(order: OrderModel(orderId: UUID().uuidString,
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 4) {
                Image(systemName: "clock")
                    ...
                    .frame(width: 16)
                    .foregroundColor(Color(R.color.gray2.name))
                let startTime = viewModel.order.orderShootingTime?.min { timeToMinutes($0) < timeToMinutes($1) } ?? ""
                Text(startTime)
                    .font(.subheadline)
