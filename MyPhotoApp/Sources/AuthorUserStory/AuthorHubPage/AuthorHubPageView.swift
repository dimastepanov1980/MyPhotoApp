...
struct AuthorHubPageView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showAddOrderView) {
            NavigationStack {
                AuthorAddOrderView(with: AuthorAddOrderViewModel(order: DbOrderModel(order: AuthorOrderModel(orderId: "", orderCreateDate: Date(), orderPrice: "", orderStatus: "", authorName: "", authorSecondName: "", instagramLink: "", authorLocation: "", description: "", orderShootingDate: Date(), orderShootingTime: [], orderShootingDuration: "", orderSamplePhotos: [], orderMessages: nil))), showAddOrderView: $showAddOrderView, mode: .new)
            }
        }
